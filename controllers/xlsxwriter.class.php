...
class XLSXWriter
{
	...
	public function writeSheetHeader($sheet_name, array $header_types, array $col_options = [])
	{
		...
		if (is_bool($col_options) && $col_options === true) {
			self::log("Warning! passing suppress_row=true to writeSheetHeader() is deprecated, this will be removed in a future version.");
			$suppress_row = true;
		}
		...
		if (isset($col_options['title_row'])) {
			$title_row = (array) $col_options['title_row'];
			$title_style = isset($col_options['title_style']) ? $col_options['title_style'] : null;

			$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="0" r="' . (1) . '">');
			foreach ($title_row as $c => $v) {
				$cell_style_idx = empty($title_style) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle('GENERAL', json_encode(isset($title_style[0]) ? $title_style[$c] : $title_style));
		...
		if (!$suppress_row) {
			...
			foreach ($header_row as $c => $v) {
				$cell_style_idx = empty($col_options['style']) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle('GENERAL', json_encode(isset($col_options['style'][0]) ? $col_options['style'][$c] : $col_options['style']));
